...
const ProductDetail = () => {
  ...
  const getFinalPrice = () => {
    if (product?.has_variant) {
      ...
      const prices = product.variants?.map((v) => parseFloat(v.final_price)).filter((v) => !isNaN(v) && v > 0);
  ...
  const getOriginalPrice = () => {
    if (product?.has_variant) {
      ...
      const prices = product.variants?.map((v) => parseFloat(v.original_price)).filter((v) => !isNaN(v) && v > 0);
    ...
    <div className='bg-bg-secondary py-8'>
      <div className='container mx-auto px-4'>
        ...
        <div className='bg-white rounded-lg shadow-sm mt-8 p-6'>
          <Tabs defaultActiveKey='description' activeKey={activeTab} onChange={(key) => setActiveTab(key)}>
            ...
            >
              <div className='py-4'>
                ...
                {!product.has_variant && (
                  <PriceTrendChart productId={product.id} isVariant={false} title='Product Price Fluctuation History' />
                )}
